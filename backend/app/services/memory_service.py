...
def retrieve_similar_interactions(
    ...
    for past_question, metadata in zip(
            documents,
            metadatas
    ):

        # Skip past interactions that were explicitly marked
        # unhelpful - no point reinforcing an answer the user
        # already rejected.
        if (
            metadata.get("helpful") == False
            or metadata.get("helpful") == "false"
        ):
            continue
...
def get_preferences_text(
    ...
    return (
        "User preferences (apply these to your answer):\n"
        + "\n".join(lines)
    )
...
def record_feedback(
    ...
    # Also tag the matching semantic memory (if we can find it)
    # so future retrieval knows this interaction was good or bad.
    _tag_semantic_memory_feedback(
        session_id,
        question,
        helpful
    )
...
def _tag_semantic_memory_feedback(
        session_id,
        question,
        helpful
):
